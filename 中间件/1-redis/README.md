...
Redis Cluster属于无中心化的集群，每个客户端直接请求节点

每个节点通过PING-PONG机制彼此互联

请求路由：redis客户端需要使用为集群的sdk才能支持协议，每个服务端节点保存了所有节点的槽信息

灵活扩容缩容：本质就是槽和对应数据在不同节点直接的移动
...
主观下线：节点与节点最后通信时间超过cluster-nodetimeout，则判断为主观下线

客观下线：半数以上持有槽的主节点，都标记某节点下线，则为客观下线

> 1、节点与节点消息中有其他节点的pfail状态，如果是主节点，则更新clusterNode内部下线报告链表
>
> 2、当报告数量大于数量一半时，标记为客观下线
>
> 3、向集群广播一条fail消息，通知所有节点标记节点为客观下线

选举

> 1、检查从节点替换故障主节点的资格：cluster-node-timeout * cluster-slave-validity-factor超过则不具备资格
>
> 2、从节点根据自身复制偏移量设置延迟选举时间，保证延迟低的优先
>
> 3、投片超过主节点数量N的N/2+1则投票成功
>
> 4、从节点执行clusterDelSlot操作，撤销故障主节点的槽。并执行clusterAddSlot把槽委派给自己
>
> 5、广播自己的pong消息，表明替换了故障主节点
